...
Joint::Joint()
{
    ...
    this->_serial = new Serial(nh, source_id, node_id, serial_port);
    this->_serial->serial_init(serial_port);
}
...
double Joint::readAngle()
{
    ...
    const float current_pos = this->_serial->serial_read();
    //ROS_INFO("readAngle");
